...
tap a button. 200ms round trip to a server. open an app. another 200ms. scroll down. another one. chain five actions together and you're waiting a full second just for the model to think<Sidenote id="2">and that's assuming good connectivity. on spotty networks or in regions with high latency to cloud providers, it's even worse.</Sidenote>. on someone else's computer.
...
a 50M parameter model trained specifically on mobile ui interactions will outperform a 70B general model at these tasks<Sidenote id="3">we've seen this pattern before. specialized small models beat general large models in vision, speech, and translation. mobile ui is next.</Sidenote>. every time. because it's built for exactly this.
