...
import { DecisionToggle, FlipCards, DecisionForm, AgentSimulation, CommitCompare, RevealQuote, Sidenote } from "@components/interactive";
...
Git commit + PR description + linked issue = decision trace for code<Sidenote id="1">this is why good commit messages matter. `fix bug` tells you nothing. `fix: skip retry on 401 because upstream returns stale tokens` tells you everything.</Sidenote>.
...
Friction. In the moment, no one wants to fill out a **"reason"** field. They're busy. They'll do it later<Sidenote id="2">this is the same reason people skip writing tests. the cost feels high now, the payoff feels far away. until it isn't.</Sidenote>. They never do.
...
**Incentives finally align.** AI agents need past decisions to get smarter<Sidenote id="3">this is basically reinforcement learning from human decisions. every "why" you capture becomes a training signal for better future choices.</Sidenote>. If you never saved why exceptions were made, the agent can't learn when to make them. Bad data in, bad decisions out.
